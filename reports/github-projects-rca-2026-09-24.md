...
### v0.8.2: the API and the web page disagree on filters

0.8.0 and 0.8.1 wrote the Lifecycle filter as `has:"Work phase"`. GitHub's API accepts it, and the reconciler's board check, which asks the API, passed. GitHub's web page rejects it ("Invalid value "Work phase" for has"): Chrome showed every card under a warning, and Safari rendered the four upgraded IOMachines Projects completely blank. The read-only probe behind the filter choice exercised the API only and was wrongly treated as proof of what the page renders. 0.8.2 writes `has:work-phase`, which both accept, replaces the quoted filter automatically on the next run, and the simulator's view of what a person sees now rejects what the web page rejects. The board check still cannot see the web page itself; a person opening the board after the first run of a new filter form remains the final check.
